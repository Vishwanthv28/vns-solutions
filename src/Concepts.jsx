  ...
  return <div className="concept sonic-concept">
    ...
    <main id="sonic-top">
      <section className="sonic-hero">
        ...
        <aside className="sonic-book-card" id="sonic-book" aria-labelledby="sonic-book-title"><div className="sonic-book-head"><p className="sonic-eyebrow">Quick request</p><h2 id="sonic-book-title">How can we help?</h2><p>Send a basic appointment request. The centre would contact you to confirm.</p></div><div className="sonic-request-tabs" role="group" aria-label="Request type"><button type="button" className={requestType === "centre" ? "active" : ""} onClick={()=>{setRequestType("centre");setRequestState("idle")}}>Centre visit</button><button type="button" className={requestType === "home" ? "active" : ""} onClick={()=>{setRequestType("home");setRequestState("idle")}}>Home collection</button></div><form onSubmit={handleRequest}><label>Full name<input required name="name" autoComplete="name" placeholder="Your name"/></label><label>Mobile number<input required name="phone" type="tel" inputMode="tel" autoComplete="tel" pattern="[0-9+() -]{10,18}" placeholder="+91 98765 43210"/></label><label>Service<select required name="service" defaultValue=""><option value="" disabled>Select a service</option>{services.map(([title])=><option key={title}>{title}</option>)}</select></label><label>Preferred day<select required name="day" defaultValue=""><option value="" disabled>Select a day</option><option>Today</option><option>Tomorrow</option><option>Another day</option></select></label><button className="sonic-primary" type="submit">Request a callback <span>→</span></button>{requestState === "sent" && <p className="sonic-form-success" role="status">Demo completed. No information was sent. The official version would securely notify the centre for confirmation.</p>}</form><small className="sonic-form-note">{requestType === "home" ? "Home collection is a proposed feature. Availability, eligible tests, area and charges require centre approval." : "This is a non-functional concept form and does not create an appointment."}</small></aside>
      </section>
